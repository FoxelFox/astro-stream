...
import vertexShader from "./poly.vertex.wgsl" with {type: "text"};
import fragmentShader from "./poly.fragment.wgsl" with {type: "text"};
import {Camera} from "../camera";
import {Poly} from "../../../shared/node/2D/poly";

export class PolyPass {

	polys: Poly[] = []
	polysNeedUpdate: boolean = true
	...
	init() {
		this.pipeline = device.createRenderPipeline({
			label: 'Poly Render Pipeline',
			layout: 'auto',
			vertex: {
				module: device.createShaderModule({
					label: 'Poly Vertex Shader',
					code: vertexShader
				}),
				buffers: [{
	...
	updateBuffers(camera: Camera, fullUpdate?: boolean) {
		...
		const matrices = new Float32Array(16 * this.polys.length);
	...
	add(node: Poly) {
		this.polys.push(node);
		this.polysNeedUpdate = true;
	}

	remove(id: number) {
		const i = this.polys.findIndex(c => c.id === id);
		if (i === -1) {
			return;
		}
